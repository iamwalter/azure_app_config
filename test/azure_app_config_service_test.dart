...
void main() {
  test('when the connection string does not contain all required values.', () {
    expect(
      () => AzureAppConfig(connectionString: 'server=test;hello=bye;up=down;'),
  ...
  group('setKeyValue()', () {
    ...
    test(
      'should call client with correct arguments and headers when empty',
      () async {
        final client = MockClient();
        final service = AzureAppConfigImpl(client: client);

        await service.setKeyValue(key: key, label: label);
  ...
  test('getKeyValue should return a KeyValue', () async {
    ...
    dioAdapter.onGet('$endpoint/kv/$key', (server) {
      return server.reply(200, testKeyValue.toJson());
    }, queryParameters: {'label': label, 'api_version': '1.0'});
  ...
  test('getKeyValues should return keyValues', () async {
    ...
    dioAdapter.onGet('$endpoint/kv', (server) {
      return server.reply(200, {
        'items': [testKeyValue.toJson(), testKeyValue.toJson()],
      });
    }, queryParameters: {'api_version': '1.0'});
  ...
  test('getKeyValues should return all keyvalues with nextlink', () async {
    ...
    final expected = [item1, item2, item3, item3, item2, item1];

    dioAdapter
      ..onGet('$endpoint/kv', (server) {
        return server.reply(200, {
          'items': [item1, item2]..map((e) => e.toJson()),
          '@nextLink': '/kv/myNextLink',
        });
      }, queryParameters: {'api_version': '1.0'})
      ..onGet('$endpoint/kv/myNextLink', (server) {
        return server.reply(200, {
  ...
  test('getFeatureFlags should retrieve feature flags', () async {
    ...
    final keyValueWithFeatureFlag = testKeyValue.copyWith(
      value: json.encode(testFeatureFlag.toJson()),
    );

    dioAdapter.onGet('$endpoint/kv', (server) {
      return server.reply(200, {
        'items': [keyValueWithFeatureFlag.toJson(), testKeyValue.toJson()],
      });
    }, queryParameters: {'api_version': '1.0'});
  ...
  test(
    ...
    () async {
      ...
      final actual = await service.findKeyValuesBy(key: 'key*', label: 'CZ');
      final expected = [key1, key2];
  ...
  test(
    '''findKeyBy should call correct endpoint & return AzureKeys''',
    () async {
      dioAdapter.onGet(
        '$endpoint/keys',
        (server) {
          return server.reply(200, {
            'items': [testKey.toJson(), testKey.toJson(), testKey.toJson()],
          });
        },
